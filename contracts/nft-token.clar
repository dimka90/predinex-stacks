...
(define-public (batch-transfer (token-ids (list 10 uint)) (recipients (list 10 principal)))
  (let ((sender tx-sender))
    (asserts! (is-eq (len token-ids) (len recipients)) ERR-INVALID-TOKEN-ID)
    (ok (map batch-transfer-helper token-ids recipients))
  )
)

(define-private (batch-transfer-helper (token-id uint) (recipient principal))
  (let ((owner (unwrap-panic (nft-get-owner? predinex-nft token-id))))
    (asserts! (is-eq tx-sender owner) ERR-NOT-OWNER)
    (try! (nft-transfer? predinex-nft token-id owner recipient))
    (map-set token-owners token-id recipient)
    (map-delete token-approvals token-id)
