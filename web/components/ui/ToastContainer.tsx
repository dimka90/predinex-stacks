    ...
    return (
        <div className="fixed bottom-8 right-8 z-[100] flex flex-col gap-4 max-w-sm w-full pointer-events-none">
            {toasts.map((toast) => (
                ...
                >
                    <div className={`mt-0.5 ${toast.type === 'success' ? 'text-green-500' : toast.type === 'error' ? 'text-red-500' : 'text-primary'}`}>
                    ...
                    <div className={`absolute bottom-0 left-0 h-1 transition-all duration-[5000ms] ease-linear w-full ${toast.type === 'success' ? 'bg-green-500' : toast.type === 'error' ? 'bg-red-500' : 'bg-primary'
                        }`} />
                </div>
            ))}
        </div>
